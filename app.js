// ============================================
// IPUC LA FONDA - app.js v5.0 COMPLETO
...
function actualizarUsuario(id, datos) { const u = db.cargar('usuarios'); const idx = (u?.usuarios || []).findIndex(x => x.id === id); if (idx < 0) return { error: 'No encontrado' }; ['nombre', 'apellidos', 'celular', 'direccion', 'ministerio', 'foto', 'estado'].forEach(c => { if (datos[c] !== undefined) u.usuarios[idx][c] = datos[c]; }); db.guardar('usuarios', u); return { mensaje: 'Actualizado' }; }
function verificarUsuario(id) { const u = db.cargar('usuarios'); const idx = (u?.usuarios || []).findIndex(x => x.id === id); if (idx < 0) return { error: 'No encontrado' }; u.usuarios[idx].verificado = true; if (!u.usuarios[idx].insignias.includes('Cuenta Verificada')) u.usuarios[idx].insignias.push('Cuenta Verificada'); db.guardar('usuarios', u); return { mensaje: 'Verificado' }; }
function cambiarPassword(id, pwActual, pwNueva) { if (pwNueva.length < 8) return { error: 'Mínimo 8 caracteres' }; const u = db.cargar('usuarios'); const idx = (u?.usuarios || []).findIndex(x => x.id === id); if (idx < 0) return { error: 'No encontrado' }; if (u.usuarios[idx].password !== db.hashPassword(pwActual)) return { error: 'Contraseña actual incorrecta' }; u.usuarios[idx].password = db.hashPassword(pwNueva); db.guardar('usuarios', u); return { mensaje: 'Contraseña actualizada' }; }
...
// ============================================
function obtenerAsistencia(uid = null) { const r = db.getAsistencia(); return uid ? r.filter(x => x.usuario_id === uid) : r; }
...
// ============================================
function obtenerVersiculoDiario() { const data = db.cargar('versiculos'); const hoy = new Date().toISOString().split('T')[0]; let actual = data?.versiculo_actual; if (!actual || actual.fecha !== hoy) { const lista = data?.versiculos || []; if (lista.length > 0) { actual = { ...lista[new Date().getDay() % lista.length], fecha: hoy }; data.versiculo_actual = actual; db.guardar('versiculos', data); } else actual = { texto: "Porque de tal manera amó Dios al mundo...", referencia: "Juan 3:16", tipo: "promesa", fecha: hoy }; } return actual; }
...
// ============================================
function obtenerNoticias() { const n = db.getNoticias(); return n.filter(x => x.estado === 'publicado').sort((a, b) => new Date(b.fecha_publicacion) - new Date(a.fecha_publicacion)); }
...
// ============================================
function obtenerEventos() { const e = db.getEventos(); const hoy = new Date().toISOString().split('T')[0]; return e.filter(x => x.fecha >= hoy).sort((a, b) => a.fecha.localeCompare(b.fecha)); }
function crearEvento(datos) { return db.addEvento(datos); }

// ============================================
// FUNCIONES DE PETICIONES
// ============================================
function obtenerPeticiones() { const p = db.getPeticiones(); return p.sort((a, b) => new Date(b.fecha) - new Date(a.fecha)); }
function crearPeticion(datos) { return db.addPeticion(datos); }

// ============================================
// FUNCIONES DE NOTIFICACIONES
// ============================================
function obtenerNotificaciones() { return db.getNotificaciones(); }
...
// ============================================
function obtenerPublicaciones() { return db.getPublicaciones(); }
function crearPublicacion(datos) { return db.addPublicacion(datos); }
function eliminarPublicacion(id) { return db.deletePublicacion(id); }
function getComentariosPublicacion(publicacionId) { return db.getComentarios(publicacionId); }
function agregarComentario(datos) { return db.addComentario(datos); }
function toggleReaccion(publicacionId, usuarioId, tipo) { return db.toggleReaccion(publicacionId, usuarioId, tipo); }
function getReaccionUsuario(publicacionId, usuarioId) { return db.getReaccionUsuario(publicacionId, usuarioId); }
...
function exportarDatos() { const datos = db.exportarTodo(); const blob = new Blob([JSON.stringify(datos, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `ipuc_backup_${new Date().toISOString().split('T')[0]}.json`; a.click(); URL.revokeObjectURL(url); return true; }
function importarDatos(archivo) { return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = function(e) { try { const datos = JSON.parse(e.target.result); db.importarTodo(datos); resolve(true); } catch (err) { reject(err); } }; reader.onerror = reject; reader.readAsText(archivo); }); }
...
console.log('✅ app.js v5.0 completo cargado - Todas las funciones listas');
