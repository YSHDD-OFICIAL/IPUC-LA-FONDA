...
class Database {
    ...
    _crearRespaldo(nombreArchivo, datosAnteriores) {
        ...
        } catch (e) { console.warn(`⚠️ Error al crear respaldo de ${nombreArchivo}:`, e); }
    }
    ...
    _recuperarRespaldo(nombreArchivo) {
        ...
        for (const claveRespaldo of respaldos) {
            try {
                ...
                if (datos) { const parsed = JSON.parse(datos); this.guardar(nombreArchivo, parsed); return parsed; }
            } catch (e) { console.error(`❌ Error al leer respaldo ${claveRespaldo}:`, e); }
        }
        return null;
    }
    ...
    hashPassword(password) {
        let hash = 0; const str = password + 'ipuc5_salt_2026';
    ...
    crearPrimerAdministrador(datos) {
        ...
        for (const c of campos) { if (!datos[c] || !String(datos[c]).trim()) return { error: `El campo '${c}' es obligatorio` }; }
        ...
        const admin = { id: 1, nombre: datos.nombre.trim(), apellidos: datos.apellidos.trim(), correo: datos.correo.trim().toLowerCase(), celular: (datos.celular || '').trim(), usuario: datos.usuario.trim().toLowerCase(), password: this.hashPassword(datos.password), foto: 'assets/avatars/admin.png', rol: 'admin', verificado: true, fecha_registro: new Date().toISOString(), estado: 'activo', ministerio: datos.ministerio || 'Pastoral', insignias: ['Administrador', 'Cuenta Verificada'] };
        if (!admins.administradores) admins.administradores = []; admins.administradores.push(admin); admins.ultimo_id = 1;
        if (this.guardar('administradores', admins)) { const cfg = this.cargar('configuracion'); if (cfg?.aplicacion) { cfg.aplicacion.primer_administrador_creado = true; this.guardar('configuracion', cfg); } return { exito: true, admin }; }
    ...
    login(usuario, password) {
        ...
        if (admin) { if (admin.estado !== 'activo') return { error: 'Cuenta desactivada' }; const { password: _, ...a } = admin; return { token: 't5_' + Date.now(), rol: 'admin', usuario: a }; }
        ...
        if (user) { if (user.estado !== 'activo') return { error: 'Cuenta desactivada' }; const { password: _, ...u } = user; return { token: 't5_' + Date.now(), rol: 'usuario', usuario: u }; }
    ...
    registrarUsuario(datos) {
        ...
        const nuevo = { id: (usuarios?.usuarios?.length || 0) + 1, nombre: datos.nombre.trim(), apellidos: datos.apellidos.trim(), documento: datos.documento.trim(), fecha_nacimiento: datos.fecha_nacimiento, sexo: datos.sexo, correo: datos.correo.trim().toLowerCase(), celular: datos.celular.trim(), direccion: (datos.direccion || '').trim(), ministerio: datos.ministerio, usuario: datos.usuario.trim().toLowerCase(), password: this.hashPassword(datos.password), foto: datos.foto || 'assets/avatars/default.png', rol: 'usuario', verificado: false, fecha_registro: new Date().toISOString(), estado: 'activo', insignias: ['Nuevo Miembro'] };
        if (!usuarios.usuarios) usuarios.usuarios = []; usuarios.usuarios.push(nuevo); usuarios.ultimo_id = nuevo.id;
        if (this.guardar('usuarios', usuarios)) { this.actualizarEstadisticasUsuarios(); return { exito: true, usuario: { id: nuevo.id, nombre: nuevo.nombre, usuario: nuevo.usuario } }; }
        return { error: 'Error al guardar' };
    }
    ...
    addPublicacion(d) {
        ...
        const nueva = { id: Date.now(), usuario_id: d.usuario_id, autor: d.autor, usuario: d.usuario, foto_autor: d.foto_autor || 'assets/avatars/default.png', verificado: d.verificado || false, contenido: d.contenido, imagen: d.imagen || '', fecha: new Date().toISOString(), reacciones: { amen: 0, me_gusta: 0, fuego: 0, orando: 0, bendicion: 0 }, comentarios_count: 0 };
        if (!p.publicaciones) p.publicaciones = []; p.publicaciones.unshift(nueva); p.ultimo_id = nueva.id;
        this.guardar('publicaciones', p); this.addNotificacion({ titulo: '📝 Nueva publicación', mensaje: `${d.autor} ha publicado en el muro`, tipo: 'publicacion' });
        return nueva;
    }
    
    updatePublicacion(id, datos) {
        const p = this.cargar('publicaciones'); const idx = (p?.publicaciones || []).findIndex(x => x.id === id);
        if (idx < 0) return false;
        Object.assign(p.publicaciones[idx], datos); this.guardar('publicaciones', p); return true;
    }
    
    deletePublicacion(id) {
        const p = this.cargar('publicaciones'); p.publicaciones = (p.publicaciones || []).filter(x => x.id !== id);
        const c = this.cargar('comentarios'); c.comentarios = (c.comentarios || []).filter(x => x.publicacion_id !== id);
        this.guardar('publicaciones', p); this.guardar('comentarios', c); return true;
    }
    ...
    addComentario(d) {
        ...
        const nuevo = { id: Date.now(), publicacion_id: d.publicacion_id, usuario_id: d.usuario_id, autor: d.autor, usuario: d.usuario, foto_autor: d.foto_autor || 'assets/avatars/default.png', contenido: d.contenido, fecha: new Date().toISOString() };
        if (!c.comentarios) c.comentarios = []; c.comentarios.push(nuevo); c.ultimo_id = nuevo.id;
        this.guardar('comentarios', c);
        const p = this.cargar('publicaciones'); const pub = (p?.publicaciones || []).find(x => x.id === d.publicacion_id);
        if (pub) { pub.comentarios_count = (pub.comentarios_count || 0) + 1; this.guardar('publicaciones', p); }
        return nuevo;
    }
    ...
    // REACCIONES
    // ============================================
    getReacciones() { return this.cargar('reacciones')?.reacciones || {}; }
    
    toggleReaccion(publicacionId, usuarioId, tipo) {
        const r = this.cargar('reacciones'); if (!r.reacciones) r.reacciones = {};
        const clave = `${publicacionId}_${usuarioId}`; const actual = r.reacciones[clave];
        if (actual === tipo) { delete r.reacciones[clave]; const p = this.cargar('publicaciones'); const pub = (p?.publicaciones || []).find(x => x.id === publicacionId); if (pub && pub.reacciones[tipo] > 0) pub.reacciones[tipo]--; this.guardar('publicaciones', p); }
        else { if (actual) { const p = this.cargar('publicaciones'); const pub = (p?.publicaciones || []).find(x => x.id === publicacionId); if (pub && pub.reacciones[actual] > 0) pub.reacciones[actual]--; this.guardar('publicaciones', p); } r.reacciones[clave] = tipo; const p = this.cargar('publicaciones'); const pub = (p?.publicaciones || []).find(x => x.id === publicacionId); if (pub) pub.reacciones[tipo] = (pub.reacciones[tipo] || 0) + 1; this.guardar('publicaciones', p); }
        this.guardar('reacciones', r); return r.reacciones[clave] || null;
    }
    
    getReaccionUsuario(publicacionId, usuarioId) {
        const r = this.cargar('reacciones'); return r?.reacciones?.[`${publicacionId}_${usuarioId}`] || null;
    }
    ...
    actualizarEstadisticasUsuarios() {
        ...
        const pub = this.getPublicaciones(); const com = this.cargar('comentarios')?.comentarios || [];
    ...
    getEstadisticas() {
        const pub = this.getPublicaciones(); const com = this.cargar('comentarios')?.comentarios || [];
        return { usuarios: (this.cargar('usuarios')?.usuarios || []).length, noticias: this.getNoticias().length, eventos: this.getEventos().length, asistencia: this.getAsistencia().length, peticiones: this.getPeticiones().length, noLeidas: this.getNoLeidas(), publicaciones: pub.length, comentarios: com.length };
    }
    
    exportarTodo() {
        const datos = {};
        for (let i = 0; i < localStorage.length; i++) { const k = localStorage.key(i); if (k.startsWith(this.prefix)) datos[k] = localStorage.getItem(k); }
        return datos;
    }
    
    importarTodo(datos) {
        for (const [k, v] of Object.entries(datos)) { if (k.startsWith(this.prefix)) localStorage.setItem(k, v); }
