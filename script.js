...
// FUNCIONES DE TOAST
// ============================================
function showToast(m, tipo, duracion) {
    tipo = tipo || 'info';
...
function confirmarAccion(ti, me, cb, tipo) {
    tipo = tipo || 'warning';
...
function actualizarSidebarUsuario() {
    ...
    if (mini) {
        ...
        var status = mini.querySelector('.user-status');
...
function actualizarContador() {
    ...
    var m = document.getElementById('contador-minutos');
    var s = document.getElementById('contador-segundos');
    var t = document.getElementById('contador-titulo');
    var e = document.getElementById('contador-estado');
    if (!d && !t) return;
    try {
        var ahora = new Date();
...
function actualizarFechaHora() {
    try {
        var a = new Date();
        var fe = document.getElementById('fecha-actual');
        var ho = document.getElementById('hora-actual');
        if (fe) {
...
function cargarVersiculoDiario() {
    var container = document.getElementById('versiculo-content');
    if (!container) return;
    var versiculos = [
    ...
    var v = versiculos[new Date().getDay() % versiculos.length];
    container.innerHTML = 
...
function confirmarAsistencia(estado) {
    var tipo = document.querySelector('input[name="tipo-asistente"]:checked')?.value || 'Hermano';
...
function compartirVersiculo() {
    ...
    var v = versiculos[new Date().getDay() % versiculos.length];
    var texto = '"' + v.texto + '" - ' + v.referencia;
    if (navigator.share) {
        navigator.share({ title: 'IPUC LA FONDA - Versículo del Día', text: texto, url: window.location.href })
            .catch(function() {});
    } else if (navigator.clipboard) {
        navigator.clipboard.writeText(texto).then(function() { showToast('Versículo copiado', 'success'); })
...
function cargarInicio(c) {
    ...
    var pubRecientes = APP_STATE.publicaciones.slice(0, 3);
    var eventos = Array.isArray(APP_STATE.eventos) ? APP_STATE.eventos : [];
    var eventosProximos = eventos.slice(0, 3);
        ...
        '<div class="fade-in">' +
            ...
            '<div class="card" style="margin-top:12px;">' +
                ...
                '<div style="margin-top:8px;">' +
                    pubRecientes.map(function(p) {
                        return '<div style="padding:8px 0;border-bottom:1px solid var(--gris-medio);">' +
            ...
            '<div class="card" style="margin-top:12px;">' +
                ...
                '<div style="margin-top:8px;">' +
                    eventosProximos.map(function(e) {
                        return '<div style="padding:8px 0;border-bottom:1px solid var(--gris-medio);display:flex;justify-content:space-between;align-items:center;">' +
                            '<div>' +
...
function cargarHorarios(c) {
    ...
    var idx = diaActual === 0 ? 6 : diaActual - 1;
                    ...
                    return '<div class="card" style="border-left:4px solid ' + (i === idx ? 'var(--azul-primario)' : 'var(--gris-medio)') + ';">' +
                        '<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;">' +
                            '<div>' +
                                ...
                                (d.cultos.length ? 
                                    d.cultos.map(function(x) {
                                        return '<div style="display:flex;align-items:center;gap:8px;color:var(--gris-texto);">' +
...
function cargarAsistencia(c) {
    var proximoCulto = CONFIG.DIAS_SEMANA[new Date().getDay() === 0 ? 6 : new Date().getDay() - 1];
    var asistencias = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.ASISTENCIAS) || '[]');
        ...
        '<div class="fade-in">' +
            ...
            '<div class="card" style="margin-top:12px;">' +
                ...
                (APP_STATE.asistencias.length === 0 ? 
                    ...
                    APP_STATE.asistencias.slice(-5).reverse().map(function(a) {
                        return '<div style="padding:6px 0;border-bottom:1px solid var(--gris-medio);display:flex;justify-content:space-between;">' +
...
function cargarNoticias(c) {
    var noticias = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.NOTICIAS) || '[]');
    ...
    c.innerHTML = 
        '<div class="fade-in">' +
            ...
            (APP_STATE.noticias.length === 0 ? 
                ...
                APP_STATE.noticias.map(function(n) {
...
function crearNoticia() {
    var titulo = prompt('Título de la noticia:');
    if (!titulo) return;
    var contenido = prompt('Contenido:');
    if (!contenido) return;
    var noticias = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.NOTICIAS) || '[]');
    var nueva = {
        id: Date.now(),
...
function eliminarNoticia(id) {
    confirmarAccion('¿Eliminar noticia?', 'Esta acción no se puede deshacer.', function() {
        var noticias = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.NOTICIAS) || '[]');
...
function cargarEventos(c) {
    var eventos = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.EVENTOS) || '[]');
    ...
    c.innerHTML = 
        '<div class="fade-in">' +
            ...
            (APP_STATE.eventos.length === 0 ? 
                ...
                APP_STATE.eventos.map(function(e) {
...
function crearEvento() {
    var titulo = prompt('Título del evento:');
    if (!titulo) return;
    var descripcion = prompt('Descripción:');
    if (!descripcion) return;
    var fecha = prompt('Fecha (YYYY-MM-DD):') || new Date().toISOString().split('T')[0];
    var hora = prompt('Hora:') || '10:00 AM';
    var eventos = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.EVENTOS) || '[]');
    var nuevo = {
        id: Date.now(),
        titulo: titulo,
...
function eliminarEvento(id) {
    confirmarAccion('¿Eliminar evento?', 'Esta acción no se puede deshacer.', function() {
        var eventos = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.EVENTOS) || '[]');
...
function cargarChat(c) {
    var chat = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.CHAT) || '[]');
            ...
            '<div class="card" style="height:400px;display:flex;flex-direction:column;">' +
                '<div style="flex:1;overflow-y:auto;padding:10px;" id="chat-messages">' +
                    (APP_STATE.chat.length === 0 ? 
                        ...
                        APP_STATE.chat.map(function(m) {
                            return '<div style="margin-bottom:8px;padding:8px 12px;border-radius:8px;' + (m.usuario_id === APP_STATE.usuario?.id ? 'background:var(--azul-surface);text-align:end;' : 'background:var(--gris-claro);') + '">' +
...
function cargarDirectorio(c) {
    var directorio = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.DIRECTORIO) || '[]');
    if (!Array.isArray(directorio)) APP_STATE.directorio = [];
    else APP_STATE.directorio = directorio;

    var miembros = APP_STATE.directorio.length ? APP_STATE.directorio : [
...
function cargarPeticiones(c) {
    var peticiones = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.PETICIONES) || '[]');
    ...
    c.innerHTML = 
        '<div class="fade-in">' +
            ...
            (APP_STATE.peticiones.length === 0 ? 
                ...
                APP_STATE.peticiones.map(function(p) {
                    return '<div class="card" style="margin-bottom:12px;border-left:4px solid ' + (p.estado === 'activa' ? 'var(--azul-primario)' : 'var(--gris-medio)') + ';">' +
                        '<div style="display:flex;justify-content:space-between;align-items:start;flex-wrap:wrap;">' +
                            '<div>' +
...
function crearPeticion() {
    var motivo = prompt('Motivo de la petición:');
    if (!motivo) return;
...
function orarPeticion(id) {
    var peticiones = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.PETICIONES) || '[]');
    var p = peticiones.find(function(p) { return p.id === id; });
    if (p) {
...
function cargarEncuestas(c) {
    var encuestas = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.ENCUESTAS) || '[]');
    ...
    c.innerHTML = 
        '<div class="fade-in">' +
            ...
            (APP_STATE.encuestas.length === 0 ? 
                ...
                APP_STATE.encuestas.map(function(e) {
                    return '<div class="card" style="margin-bottom:12px;border-left:4px solid ' + (e.activa ? 'var(--exito)' : 'var(--gris-medio)') + ';">' +
...
function crearEncuesta() {
    var titulo = prompt('Título de la encuesta:');
    if (!titulo) return;
    var preguntas = prompt('Preguntas (separadas por coma):');
    if (!preguntas) return;
    var encuestas = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.ENCUESTAS) || '[]');
    var nueva = {
        id: Date.now(),
        titulo: titulo,
        preguntas: preguntas.split(',').map(function(p) { return p.trim(); }),
        fecha: new Date().toISOString(),
        activa: true
    };
...
function cargarBiblioteca(c) {
    var biblioteca = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.BIBLIOTECA) || '[]');
    ...
    c.innerHTML = 
        '<div class="fade-in">' +
            ...
            '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;">' +
                (APP_STATE.biblioteca.length === 0 ?
                    '<div class="card" style="grid-column:1/-1;text-align:center;padding:30px;color:var(--gris-texto);">No hay recursos disponibles</div>' :
                    APP_STATE.biblioteca.map(function(r) {
...
function agregarRecurso() {
    var titulo = prompt('Título del recurso:');
    if (!titulo) return;
    var autor = prompt('Autor:');
    if (!autor) return;
    var categoria = prompt('Categoría:') || 'General';
    var biblioteca = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.BIBLIOTECA) || '[]');
    var nuevo = {
        id: Date.now(),
        titulo: titulo,
...
function cargarGaleria(c) {
    var galeria = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.GALERIA) || '[]');
        ...
        '<div class="fade-in">' +
            ...
            '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:12px;margin-top:16px;">' +
                (APP_STATE.galeria.length === 0 ? 
                    ...
                    APP_STATE.galeria.map(function(g) {
...
function agregarImagen() {
    var titulo = prompt('Título de la imagen:') || 'Imagen';
    ...
    c.innerHTML = 
        '<div class="fade-in">' +
            '<div style="text-align:center;padding:30px;background:linear-gradient(135deg,var(--azul-primario),var(--azul-claro));color:white;border-radius:var(--borde-radius);margin-bottom:16px;">' +
                '<img src="' + (u.foto || 'assets/avatars/default.png') + '" style="width:80px;height:80px;border-radius:50%;border:3px solid var(--dorado);object-fit:cover;">' +
...
function cargarPodcast(c) {
    var podcast = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.PODCAST) || '[]');
    ...
    c.innerHTML = 
        '<div class="fade-in">' +
            ...
            (APP_STATE.podcast.length === 0 ? 
                ...
                APP_STATE.podcast.map(function(p) {
                    return '<div class="card" style="margin-bottom:12px;border-left:4px solid var(--info);">' +
                        '<div style="display:flex;justify-content:space-between;align-items:start;flex-wrap:wrap;">' +
                            '<div>' +
...
function agregarPodcast() {
    var titulo = prompt('Título del episodio:');
    if (!titulo) return;
    var pastor = prompt('Pastor/Predicador:');
    if (!pastor) return;
    var duracion = prompt('Duración:') || '30 min';
    var podcast = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.PODCAST) || '[]');
    var nuevo = {
        id: Date.now(),
        titulo: titulo,
...
function cargarAnalytics(c) {
    var totalMiembros = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.USUARIO) || '[]').length || 0;
    var totalPublicaciones = Array.isArray(APP_STATE.publicaciones) ? APP_STATE.publicaciones.length : 0;
    var totalEventos = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.EVENTOS) || '[]').length;
    var totalAsistencias = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.ASISTENCIAS) || '[]').length;
    var totalPeticiones = JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.PETICIONES) || '[]').length;
        ...
        '<div class="fade-in">' +
            ...
            '<div class="card" style="margin-top:12px;">' +
                ...
                '<div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap;">' +
                    ['es','en','pt','fr','de','it'].map(function(l) {
                        return '<button class="btn-outline btn-sm ' + (APP_STATE.idioma === l ? 'active' : '') + '" onclick="cambiarIdioma(\'' + l + '\')">' +
    ...
    c.innerHTML = 
        '<div class="fade-in">' +
            ...
            '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:16px;">' +
                ['usuarios', 'publicaciones', 'eventos', 'asistencias', 'peticiones', 'notificaciones'].map(function(key) {
                    return '<div class="card" style="text-align:center;border-left:4px solid var(--azul-primario);">' +
...
function cargarPublicaciones(c) {
    if (!Array.isArray(APP_STATE.publicaciones)) APP_STATE.publicaciones = [];
    if (typeof APP_STATE.reacciones !== 'object' || APP_STATE.reacciones === null) APP_STATE.reacciones = {};
                ...
                '<div class="card" style="margin-bottom:16px;">' +
                    ...
                    '<form id="form-publicacion">' +
                        '<div class="form-group">' +
                            '<textarea class="form-input" id="contenido-publicacion" placeholder="¿Qué quieres compartir?" rows="3" maxlength="2000" required></textarea>' +
                        ...
                        return '<div class="card" style="margin-bottom:12px;" id="pub-' + p.id + '">' +
                            '<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">' +
                                ...
                                '<div style="flex:1;">' +
                                    ...
                                    '<p style="font-size:0.75rem;color:var(--gris-texto);">@' + (p.usuario || 'usuario') + ' · ' + formatearFecha(p.fecha) + '</p>' +
                                '</div>' +
                            ...
                            '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px;padding:8px 0;border-top:1px solid var(--gris-medio);border-bottom:1px solid var(--gris-medio);">' +
                                CONFIG.REACCIONES_TIPOS.map(function(r) {
                                    var isActive = APP_STATE.reacciones[p.id + '_' + (APP_STATE.usuario?.id || '')] === r.clave;
                                    return '<button onclick="toggleReaccion(' + p.id + ',\'' + r.clave + '\')" style="padding:6px 10px;border-radius:20px;border:1px solid ' + (isActive ? 'var(--azul-primario)' : 'var(--gris-medio)') + ';background:' + (isActive ? 'var(--azul-surface)' : 'transparent') + ';cursor:pointer;font-size:0.8rem;transition:all 0.2s;">' +
                            ...
                            (APP_STATE.usuario ? 
                                '<div style="display:flex;gap:8px;">' +
                                    '<input type="text" class="form-input" id="comentario-' + p.id + '" placeholder="Escribe un comentario..." style="flex:1;padding:6px 10px;font-size:0.8rem;">' +
    ...
    var textarea = document.getElementById('contenido-publicacion');
    var contador = document.getElementById('caracteres-contador');
...
function toggleReaccion(publicacionId, tipo) {
    ...
    if (typeof APP_STATE.reacciones !== 'object' || APP_STATE.reacciones === null) APP_STATE.reacciones = {};
    var clave = publicacionId + '_' + APP_STATE.usuario.id;
    var actual = APP_STATE.reacciones[clave];
    var pub = APP_STATE.publicaciones.find(function(p) { return p.id === publicacionId; });
    if (!pub) return;
...
function agregarComentario(publicacionId) {
    ...
    var input = document.getElementById('comentario-' + publicacionId);
...
function eliminarPublicacion(id) {
    confirmarAccion('¿Eliminar publicación?', 'Esta acción no se puede deshacer.', function() {
        if (!Array.isArray(APP_STATE.publicaciones)) APP_STATE.publicaciones = [];
        APP_STATE.publicaciones = APP_STATE.publicaciones.filter(function(p) { return p.id !== id; });
        localStorage.setItem(CONFIG.STORAGE_KEYS.PUBLICACIONES, JSON.stringify(APP_STATE.publicaciones));
...
function mostrarRegistro() {
    ...
    document.getElementById('registro-form')?.addEventListener('submit', function(e) {
        ...
        if (typeof registro !== 'undefined') {
            var resultado = registro(d);
            if (resultado.success) {
                showToast('Registro exitoso. Inicia sesión', 'success');
                setTimeout(function() { mostrarLogin(); }, 1500);
            } else {
...
document.addEventListener('DOMContentLoaded', function() {
    // Cargar tema
    var temaGuardado = localStorage.getItem(CONFIG.STORAGE_KEYS.TEMA) || 'light';
    APP_STATE.tema = temaGuardado;
    aplicarTema(temaGuardado);

    // Cargar idioma
    var idiomaGuardado = localStorage.getItem('ipuc10_idioma') || 'es';
    ...
    var usuarioData = localStorage.getItem(CONFIG.STORAGE_KEYS.USUARIO);
    var rol = localStorage.getItem(CONFIG.STORAGE_KEYS.ROL);
